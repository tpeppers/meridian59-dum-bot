...
| [`max-health-bands.jsonc`](max-health-bands.jsonc) | **the fleet sorts itself.** A station says what SIZE of character it is for — `max_health: {at_least, below}` — so which room a character works follows a number on its own board row rather than a cohort list somebody maintains. It replaces two doctrines and two `--agent` scopes with one of each, and it carries the crossing: a character that grows past the boundary, or dies back through it, is reassigned and gets one last run at the counters on the way |
...
| [`desk-practice.jsonc`](desk-practice.jsonc) | **practice while running a service desk.** The desk character drills the named spells between customers and never spends below two casts of its dearest service — a reserve the harness DERIVES from the desk's menu and the character's spell book (60 mana with reveal on the menu), reagents included. No mana number in the doctrine, on purpose; the schema refuses one. Name the desk in `local/` |
