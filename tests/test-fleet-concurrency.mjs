...
test('concurrency: the default is one decision per pass, exactly as before', () => {
  const { intent, intents } = decide(set(ruleFor('a', 'acct01'), ruleFor('b', 'acct02')), {}, doctrine());
  assert.equal(intents.length, 1);
  assert.equal(intent.rule, 'a');
});

test('concurrency: raising the cap lets a later rule act on a DIFFERENT character', () => {
  const r = decide(set(ruleFor('a', 'acct01'), ruleFor('b', 'acct02')), {}, doctrine(), { max: 3 });
...
test('concurrency: but never twice on the SAME character', () => {
  const r = decide(set(ruleFor('a', 'acct01'), ruleFor('b', 'acct01')), {}, doctrine(), { max: 3 });
...
test('concurrency: an intent whose reach cannot be read is exclusive, not universal', () => {
  ...
  const r = decide(set(ruleFor('a', 'acct01'), opaque, ruleFor('c', 'acct03')), {}, doctrine(), { max: 4 });
  ...
  assert.equal(decide(set(opaque, ruleFor('c', 'acct03')), {}, doctrine(), { max: 4 })
...
test('concurrency: a rule that passes still does not consume a slot', () => {
  ...
  const r = decide(set(passer, ruleFor('a', 'acct01'), ruleFor('b', 'acct02')), {}, doctrine(), { max: 2 });
  assert.deepEqual(r.intents.map(i => i.rule), ['a', 'b']);
});

test('intentAgents reads every shape a fleet intent names characters in', () => {
  assert.deepEqual([...intentAgents({ agent: 'acct01' })], ['acct01']);
  assert.deepEqual([...intentAgents({ orders: { agent: 'acct02' } })], ['acct02']);
  ...
  assert.deepEqual([...intentAgents({ plan: [{ from: 'acct03', to: 'acct04' }] })].sort(), ['acct03', 'acct04']);
  assert.deepEqual([...intentAgents({ plan: [{ args: { agent: 'acct05' } }] })], ['acct05']);
