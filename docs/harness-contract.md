...
| `cast` with `spell: "enchant weapon"` and a **numeric** `target` | the Ukgoth Trolls dedication round: a Kraanan caster dedicates a weapon in its own pack, addressed by object id. `surface.mjs` refuses it at a name (a name could resolve to a creature or player) |
| `supply` with `what: "<one reagent name>", amount` | a reagent hand-over addressed by name, because the free board's `pack_items` carries names without ids — the depot tops up the dedicator's 3 elderberry + 1 orc tooth |
| `fleet[].weapon_magic` | which of a character's weapons bypass a troll's NONMAGIC 80, as the keeper READ them (look text, the lapse sentence): `{wielded:{name,class,bypasses_nonmagic,made}, magic_spares, unknown, weapons:[{id,name,class,bypasses_nonmagic,made,wielded}]}`. From m59-harness `b2d26f6` (m59-weapon-magic.mjs). Null on an older broker, and the troll gate refuses null |
| `sell` with exact `items` ids and a named smith (`to`) | the Ukgoth courier errand: the depot's surplus REAL weapons, sold to Quintor (374) or another weapon buyer. Conjured weapons are never in the list — the harness also refuses to offer them in its own sale plans (`_madeItemIds`, m59-harness) because IA_MADE makes every merchant refuse in silence |
| `supply` with `what: [{id, amount}]` between two in-scope characters in one room | the stage room's hand-overs: surplus up to the depot, a family weapon down from it, and the courier's pick-up |
| `autopilot` `prefer_magic_weapon` | the keeper's magic tie-break: between two weapons of the same priority rank, wield the one read as magic, and swap at once on a lapse. Never outranks `weapon_priority`, so no weapon family is forced |
...
| `act` **with `verb: "go"`, or `verb: "activate"` on a Feast Hall table** | `go` is the only way to work a place-triggered square. `UserGo` (`user.kod:5656`) answers on whatever square the character is standing on, which is the mechanism behind stairs, doors, ladders and the Castle Victoria crate. `activate` is admitted for exactly the seven food dispensers the Duke's Feast Hall places (`duke4.kod:123-176`, by their in-room names): `TryActivate` (`dispensr.kod:41`) creates one item of food and hands it over, and the server checks only that the dispenser is in the same room. Every other verb, and `activate` on anything else, reaches into the pack or works a mechanism and is refused at the surface |
| `travel_estimate` (read) | how far every character is from the hall, once per distinct room; both feast doors turn on it. `cancel_movement` clears a journey that never arrived |
